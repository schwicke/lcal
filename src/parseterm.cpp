/*
 * $Id: ParseTerm.C,v 1.5 2004/01/20 19:54:33 ulrich Exp $
...
void Pot::print(){
	...
	case 'V':
	    ...
	    break;
	case 'l':
	    std::cout << "log10(" ;Klammer->print();std::cout << ")";
	    break;
	case 'e':
	    std::cout << "log(";Klammer->print();std::cout << ")";
	    break;
	case 's':
	    std::cout << "log(";Klammer->print();std::cout << ")";
	    break;
	case 'o':
	    std::cout << "cos(";Klammer->print();std::cout << ")";
	    break;
	case 't':
	    std::cout << "tan(";Klammer->print();std::cout << ")";
	    break;
	case 'a':
	    std::cout << "atan(";Klammer->print();std::cout << ")";
	    break;
	case '1':
	    std::cout << "asin(";Klammer->print();std::cout << ")";
	    break;
	case '2':
	    std::cout << "acos(";Klammer->print();std::cout << ")";
	    break;
	case '3':
	    std::cout << "sinh(";Klammer->print();std::cout << ")";
	    break;
	case '4':
	    std::cout << "cosh(";Klammer->print();std::cout << ")";
	    break;
	case '5':
	    std::cout << "tanh(";Klammer->print();std::cout << ")";
	    break;
	case 'r':
	    std::cout << "sqrt(";Klammer->print();std::cout << ")";
	    break;
    }
}
double Pot::value(){
	...
	case 'V':
	    ...
	    break;
	case 'l':
	    return log10(Klammer->value()) ;
	    break;
	case 'e':
	    return log(Klammer->value()) ;
	    break;
	case 's':
	    return sin(Klammer->value()) ;
	    break;
	case 'o':
	    return cos(Klammer->value()) ;
	    break;
	case 't':
	    return tan(Klammer->value()) ;
	    break;
	case 'a':
	    return atan(Klammer->value()) ;
	    break;
	case '1':
	    return asin(Klammer->value()) ;
	    break;
	case '2':
	    return acos(Klammer->value()) ;
	    break;
	case '3':
	    return sinh(Klammer->value()) ;
	    break;
	case '4':
	    return cosh(Klammer->value()) ;
	    break;
	case '5':
	    return tanh(Klammer->value()) ;
	    break;
	case 'r':
	    return sqrt(Klammer->value()) ;
	    break;
    }
    return(0);
}
...
Fak * Fak::ParseFak(char * &s){
    Pot * potenz = new Pot(s);
    Faktoren = new ListOf<Pot> (potenz);
    while (strlen(s)>0 && *s =='^') {
	s++;
	potenz = new Pot(s);
	Faktoren->add_elem(potenz,'^');
    };
    return(NULL);
};
double Fak::value(){
    Pot * potenz = (Faktoren->first_node())->element();
    double val = potenz->value();
    Faktoren->move_next();
    Node <Pot>* i = Faktoren->current_node();
    while (i!=0){
	val = pow(val,i->element()->value());
	Faktoren->move_next();
	i=Faktoren->current_node();
    };
    Faktoren->reset();
    return val;
};
...
int Fak::ispot(){
    return(Faktoren->is_first() & Faktoren->is_last());
};

int Fak::isconst(){
    int res = 0;
    Node<Pot> * i = Faktoren->first_node();
    while (i!=0){
...
Sum * Sum::ParseSum(char * &s){
    Fak * faktor = new Fak(s);
    Summanden = new ListOf<Fak> (faktor);
    char action;
...
void Sum::print(){
    Fak * faktor = (Summanden->first_node())->element();
    faktor->print();
    Summanden->move_next();
    Node<Fak>* i = Summanden->current_node();
    while (i !=0) {
	if (i->action == '*') {
	    std::cout << "*";
	} else {
...
int Sum::isconst(){
    int res = 0;
    Node<Fak> * i = Summanden->first_node();
    while (i!=0){
	res += (i->element())->isconst();
	Summanden->move_next();
	i = Summanden->current_node();	
